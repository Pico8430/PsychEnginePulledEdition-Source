...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		botplayTxt = new FlxText(395, timeBarBG.y + 55, FlxG.width - 800, "Botplay ON", 32);
	...
	override public function update(elapsed:Float)
	{
		...
		if(ratingName == '?') {
			scoreTxt.text = 'Score: ' + songScore + ' | Health:' + Math.floor(health * 500) / 10 + "%" + ' | Misses: ' + songMisses + ' | Rating: ' + ratingName ;
		} else {
			scoreTxt.text = 'Score: ' + songScore + ' | Health:' + Math.floor(health * 500) / 10 + "%" + ' | Misses: ' + songMisses + ' | Rating: ' + ratingName + ' (' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%)' + ' - ' + ratingFC;//peeps wanted no integer rating
		}
	...
	public function RecalculateRating() {
		...
		if(ret != FunkinLua.Function_Stop)
		{
			...
			if (goods > 0) ratingFC = "Good FC";
			if (bads > 0 || shits > 0) ratingFC = "Full Combo";
			if (songMisses > 0 && songMisses < 10) ratingFC = "SDCB";
			else if (songMisses >= 10) ratingFC = "Clear";
			if (songMisses > 50 && songMisses < 300) ratingFC = "-Clear"
			else if (songMisses >= 300) ratingFC = "Bad run";
			if (songMisses > 500 && songMisses < 1000) ratingFC = "Very bad"
			else if (songMisses >= 1000) ratingFC = "what";
		}
